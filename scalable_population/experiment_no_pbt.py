...
if __name__ == '__main__':
    ...
    # Training.
    flags.DEFINE_integer('total_environment_frames', int(40e9),
    ...
    # Loss settings.
    flags.DEFINE_float('entropy_cost', 0.05, 'Entropy cost/multiplier.')
...
class Agent(snt.RNNCore):
    ...
    def _torso(self, input_):
        ...
        f = tf.reshape(conv_out, [batch_size, LSTM_SIZE])
...
def build_learner(agent, agent_state, env_outputs, agent_outputs, optimizer, num_env_frames, learning_rate, index=0):
    ...
    if VARIABLE_ENTROPY:
        entropy_cost *= 0.85**index
    print('entropy_cost', entropy_cost)
    ...
    total_loss += entropy_cost * compute_entropy_loss(
    ...
    tf.summary.scalar('entropy_cost_' + str(index), entropy_cost)
            ...
            with tf.device('/gpu'):
                ...
                for i in range(FLAGS.num_agents):
                    ...
                    for data_from_actors in data_from_actorss:
                        ...
                        outputs.append(build_learner(agents[i], data_from_actors.agent_state,
                                                     data_from_actors.env_outputs,
                                                     data_from_actors.agent_outputs,
                                                     optimizer, num_env_frames, learning_rate, index=i))
